...
from sepsis_osc.ldm.data_loading import get_data_sets_prediction, prepare_batches
...
def calc_full_directional_loss(
    ...
    alignment = dp * dt
    penalty = penalty_weight * jax.nn.celu(-alignment)
...
def get_latent_seq(
    ...
    *,
    key: jnp.ndarray,
) -> tuple[Float[Array, "batch time latent_dim"], Float[Array, " batch"]]:
    ...
    enc_keys = jax.vmap(model.encoder.make_keys)(batch_keys)

    # --------- Prediction
    zbeta0, zsigma0, h0 = jax.vmap(model.encoder)(x[:, 0], dropout_keys=enc_keys)
    ...
    (_, _), z_preds = jax.lax.scan(predict_next, carry_init, xs=None, length=T - 1)

    # build full time sequence
    z_seq = jax.nn.sigmoid(jnp.transpose(jnp.concatenate([z0[None], z_preds], axis=0), (1, 0, 2)))  # (T, batch, z_dim)

    return (
        constrain_z(z_seq),  # (batch, T, z_dim)
        inf0.squeeze(),  # (batch, T)
    )
...
def loss(
    ...
    *,
    ...
    # infection_pred = infection_true.max(axis=-1)
    # sofa_pred = sofa_true / 24
    # --------- Matching Loss
    ...
    x_vals, x_indicator = x[..., : 50 + 2], 1 - x[..., 50 + 2 :]
    ...
    aux.sofa_d2_p = jax.vmap(prob_increase, in_axes=(0, None, None))(sofa_pred, model.d_thresh, model.d_scale)

    # sofa_true_d2_p = jax.vmap(prob_increase, in_axes=(0, None, None))(sofa_true / 23.0, model.d_thresh, model.d_scale)
    sofa_true_d2_p = (jnp.diff(sofa_true / 24.0, axis=-1) > 0.0).any(axis=-1).astype(jnp.float32)
    aux.sofa_d2_p_loss = optax.sigmoid_binary_cross_entropy(
        binary_logits(aux.sofa_d2_p), sofa_true_d2_p
    )
    infection_true_p = 1 - jnp.prod(1 - infection_true, axis=-1)
    ...
    #     1.5
    # )
    aux.susp_inf_p = jax.nn.sigmoid(infection_pred)

    aux.sep3_p = aux.sofa_d2_p * aux.susp_inf_p
    aux.sep3_p_loss = optax.sigmoid_binary_cross_entropy(
        binary_logits(aux.sep3_p), (sepsis_true == 1.0).any(axis=-1)
    )

    # --------- Directional Loss
    aux.sofa_directional_loss = jax.vmap(calc_full_directional_loss)(sofa_pred, sofa_true / 23.0)
    ...
    aux.total_loss = (
        ...
        + uncertainty_scale(jnp.mean(aux.sep3_p_loss) * params.w_sep3, model.sep3_lsigma)
        + uncertainty_scale(jnp.mean(aux.sofa_directional_loss) * params.w_sofa_direction, model.sofa_dir_lsigma)
...
if __name__ == "__main__":
    ...
    train_conf = TrainingConfig(
        batch_size=512, window_len=1 + 6, epochs=int(10e3), perc_train_set=0.333, validate_every=5
    )
    lr_conf = LRConfig(init=0.0, peak=5e-3, peak_decay=0.5, end=1e-12, warmup_epochs=1, enc_wd=1e-4, grad_norm=1.0)
    ...
    (
        ...
        test_x,
        test_y,
    ) = get_data_sets_prediction(
        window_len=train_conf.window_len, swapaxes_y=(0, 2, 1), dtype=jnp.float32, full=model_conf.w_indicator
    )
    ...
    _sofas, counts = np.unique(train_y[..., 0], return_counts=True, sorted=True)
    ...
    inf_predictor = InfectionPredictor(
        ...
        dropout_rate= 0.7, #model_conf.dropout_rate,
        dtype=dtype,
    )
    stop_inf = EarlyStopping(patience=100, direction=-1, min_steps=0)
    gru = GRUPredictor(
        key=key_predictor,
