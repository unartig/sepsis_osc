...
Training the LDM requires additional packages (sepecifically [YAIB](https://github.com/rvandewater/YAIB)), together with the [MIMIC-IV](https://physionet.org/content/mimiciv/)-database being correctly set up and the `yaib_data_dir` in 'src/sepsis_osc/utils/config.py' pointing to the correct location.
The cohort can be generated by running the [misc/yaib_cohort_script.py](misc/custom_sepsis_yaib_cohort.py).
