...
The decoder $d_theta_d$, illustrated in @fig:ae/B, is implemented as a four-layer feed-forward network that progressively up-samples from the latent representation back to the feature dimension, reconstructing only the features, not the imputation indicator.
...
For the change in #acr("SOFA") score ($Delta$#acr("SOFA"), of consecutive time-steps), the distribution is strongly centered around zero for both ground truth and predictions, reflecting that most time-steps do not involve any acute changes.
