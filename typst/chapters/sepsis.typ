...
In 2017, an estimated 48.9 million people worldwide suffered from sepsis and the same year, 11 million deaths were associated with sepsis @rudd2020global, which makes up 19.7% of yearly deaths, making it the most common cause of in-hospital deaths.
...
Quantitatively, almost half of all sepsis-related deaths occur as a secondary complication of an underlying injury or a non-communicable, also known as chronic disease @fleischmann2022sepsis.
...
The following chapter is structured around the most commonly used and widely accepted sepsis definition, which is referred to as _Sepsis-3_, it characterizes sepsis as a "life-threatening organ dysfunction caused by a dysregulated host response to infection" @Sepsis3.
Starting with @sec:sepbio, both the pathology and underlying biology of that definition are described phenomenologically.
Followed by @sec:sep3def, which provides a more medical and technical overview to this definition.
In @sec:sepwhy the necessity for reliable and clinically practical sepsis prediction systems is discussed and how these systems are utilized in clinical practice.
...
For example, Cardiomyocytes in the heart drive the contraction, relaxation and therefor blood pumping, Hepatocytes in the liver doing metabolism and detoxification and Neurons in the brain providing signal transmission @Chen2022Crosstalk.
...
The pathogen summarizes all types of organisms that can be harmful to the body, this includes germs, fungi, algae, or parasites.
When a pathogen enters the body through the skin, a mucous membrane or an open wound, the first line of nonspecific defense, the innate immune system @Fischer2022Innit, gets activated.

This rapid response does not require the body to have seen the specific pathogen before, opposed to a slower more specific and adapted immune response.
...
After the pathogen is detected, the release of cytokines is quickly initiated.
The release peaks as immune cells are recruited and automatically fades out once the initial pathogen is controlled and the host returns to a healthy and balanced state, the homeostasis.
...
At the same time, the mitochondria start to fail.
...
The Sepsis-3 definition as been created by a working group led by the "Society of Critical Care Medicine and the European Society of Intensive Care Medicine" in 2016.
...
#(
  ...
  + #text()[
      *Confirmed or Suspected Infection*, which has no strict medical definition or classification, meaning what counts as #acr("SI") remains a vague.
...
These findings confirm that earlier identification of sepsis improves clinical results, even if existing tools are not yet capable enough, and emphasize the need for developing more reliable alert systems.

A recent study suggests a paradigm shift in sepsis detection, namely from a purely symptom-based to a systems-based approach @Dobson2024Revolution.
...
Despite these promising directions, the translation of predictive models into clinical practice has proven challenging.
Studies consistently identify barriers, such as alert fatigue, where excessive false positives or clinically non-actionable alarms disrupt workflow, reduce clinician trust, and ultimately lead to ignored warnings.
Prediction systems face a fundamental trade-off: higher sensitivity captures more true cases but generates more false alarms, while higher specificity reduces alert fatigue but risks missing sepsis cases where early treatment is critical.
