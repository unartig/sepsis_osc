...
Moreover, a septic condition cannot be reduced to a single specific physiological phenomenon, instead it combines multiple complex and interdependent processes across different biological scales.
...
Starting with an explanation on how human tissue is reacting to local infections or injuries on a cellular level in @sec:cell, how this can escalate to _cytokine storms_ in @sec:storm and ending with systemic organ failure in @sec:fail.
...
The stroma not only contributes to the tissue's structure, but it also actively participates in biochemical signaling and immune regulation.
...
The #acp("PAMP") and #acp("DAMP") can be detected by pattern recognition receptors, which are found in resident immune cells, as well as stroma cells.
...
Further, cytokines are redundant, meaning targeted cells can show identical responses to different cytokines @House2007cyto, these features seem to fulfill some safety mechanism to guarantee vital communication flow.
After release, cytokines have a relatively short half-life (only a few minutes) but through cascading effects the cytokines can have substantial impact on their micro-environment.

Under normal circumstances, the release of inflammatory cytokines is tightly regulated in time and magnitude.
...
In certain scenarios a disturbance to the regulatory mechanisms triggers a positive inflammatory feedback loop, accompanied by a massive release of pro-inflammatory cytokines.
...
Ultimately, this leads to a continuous and uncontrolled release of cytokines that fails to shut down, this corresponds to the "host's dysregulated response to an infection" of the Sepsis-3 definition.
...
To this day, the fundamental principles that govern the transition from a regulated immune response to a self-destructive cytokine storm are not fully understood.
Since the 90s there has been a lot of research focused on cytokines and their role in the innate immune system and overall activation behavior @Garlanda2025.
...
#reset-acronym("SOFA")
As illustrated in the previous section, it is difficult to pinpoint the exact moment at which the immune response switches from normal to dysregulated behavior.
To classify patients as septic multiple clinically grounded definitions have been proposed over time.
...
Their resolution, named "Third International Consensus Definitions for Sepsis and Septic Shock" @Sepsis3, hence the name Sepsis-3, provides to this day the most widely used sepsis definition and guidance on sepsis identification.
...
#(
  ...
  + #text()[
      *Confirmed or Suspected Infection*, which has no strict medical definition or classification, meaning what counts as #acr("SI") remains vague.
      ...
      For retrospective data-driven classification it is suggested to characterize any patient prescribed with antibiotics followed by the cultivation of body fluids, or the other way around, with a #acr("SI") @Sepsis3.\
      The timings of prescription and fluid samplings play a crucial role.
      If the antibiotics were administered first, then the cultivation has to be done in the first 24 hours after first prescription, if the cultivation happened first, the antibiotics have to be prescribed in the following 72 hours @Sepsis3.
      These timings can be seen in the lower part of @fig:ricu (with the abbreviated ABX).
  ...
  + #text()[
      *Dysregulated Host Response* is characterized by the worsening of organ functionality over time.
      ...
      A more detailed listing of corresponding markers for each organ assessment can be found in @tab:sofa in the @a:sofa.
      The magnitude of a patient's initial #acr("SOFA")-score captures preexisting organ dysfunction.
...
An increase in #acr("SOFA") $>=2$ in the 48 hours before or 24 hours after the #acr("SI") time, the so called #acr("SI")-window, is per Sepsis-3 definition the _sepsis onset time_.
...
#(
  align(center, list(
    ...
    align(left, [the lowest value of the 24 hours previous to the increase.]),
  ))
)
...
Using the lowest #acr("SOFA") score as baseline, the increase $>=2$ for patients with #acr("SI") was associated with an 18% higher mortality rate according to @SOFAscore, a retrospective analysis based on #acr("ICU") data.
...
// Up until today, even though #acr("SOFA") was created as a clinical bedside score, some of the markers used in it are not always available to measure or at least not every 24 hours @moreno2023sofaupdate.
...
To this day, sepsis remains as an extreme burden on the worldwide healthcare system.
...
Globally, sepsis remains significantly under-diagnosed due to its nonspecific symptoms.
...
Traditional sepsis screening has relied on clinical scoring systems such as #acr("SOFA") or #acr("qSOFA"), a simplified of the #acr("SOFA")-score for faster bedside assessment but less validity for in-hospital mortality @SOFAscore.
...
This clinical reality has motivated the development of automated prediction systems that can continuously monitor patients and alert clinicians to elevated sepsis risk before organ failure develops.
...
A meta-analysis of seven sepsis early warning systems implemented in clinical practice, automating patient screening based on #acp("EHR"), found no evidence for improvement in patient outcomes, suggesting insufficient predictive power of analyzed alert systems or inadequate system integration @Alshaeba2025Effect.
