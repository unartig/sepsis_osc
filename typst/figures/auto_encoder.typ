...
#let ae_fig = canvas({
    ...
    box-node((x-center - box-width/2, y-current), box-width, box-height,
      [Input $bold(mu)_1$ #dim(input-dim)], fill: rgb("#e3f2fd"))
    y-current -= y-gap
    ...
    cline((x-center - eps, 0), (x-center - eps, -y-gap/2), (left-x, -y-gap/2), (left-x, -y-gap + eps), mark: (end: ">"))
    cline((x-center + eps, 0), (x-center + eps, -y-gap/2), (right-x, -y-gap/2), (right-x, -y-gap + eps), mark: (end: ">"))

    y-current -= 0.5

    // Features branch (left)
    box-node((left-x - 1.5, y-current), 3, box-height,
      [Features #dim(half-dim)], fill: rgb("#fff3e0"))

    // Indicators branch (right)
    box-node((right-x - 1.5, y-current), 3, box-height,
      [Indicators #dim(half-dim)], fill: rgb("#fff3e0"))

    y-current -= y-gap

    // Gating
    box-node((left-x - 2.5, y-current), 5, box-height,
      [Linear + Sigmoid #dimarr(half-dim, half-dim)], fill: rgb("#ffe0b2"))
    ...
    cline((left-x, concat-y + y-gap*1.25), (left-x, concat-y + y-gap), (x-center - eps, concat-y + y-gap), (x-center - eps, concat-y + box-height), mark: (end: ">"))
    cline((right-x, y-current + 2*y-gap + box-height - 3.4*eps),  (right-x, concat-y + y-gap),  (x-center + eps, concat-y + y-gap), (x-center + eps, concat-y + box-height), mark: (end: ">"))

    // Concatenate
    box-node((x-center - 2.75, y-current), 5.5, box-height,
      [Concatenate #if show-dims [(#half-dim + #half-dim → #input-dim)] else []], fill: rgb("#e1bee7"))
    ...
    box-node((x-center - box-width/2 - pw/2, y-current), box-width + pw, box-height*2.7,
      align(center, text()[Residual Block 1 \ LayerNorm + Linear + GELU \ #dimarr(input-dim, hidden-dim)]), fill: rgb("#c8e6c9"))
    ...
    for i in range(2) {
      ...
      ccircle((x-center, mid-y - y-gap + 3* eps), radius: 0.15, fill: luma(221))
    ...
    box-node((x-center - box-width/2 - pw/2, y-current), box-width + pw, box-height,
      [Linear + GELU #dimarr(hidden-dim, pre-head-dim)], fill: rgb("#b2dfdb"))
    ...
    cline((x-center - eps, head-split-y + y-gap - box-height), (x-center - eps, head-split-y + y-gap/4), (left-x, head-split-y + y-gap/4), (left-x, y-current + box-height), mark: (end: ">"))
    cline((x-center + eps, head-split-y + y-gap - box-height), (x-center + eps, head-split-y + y-gap/4), (right-x, head-split-y + y-gap/4), (right-x, y-current + box-height), mark: (end: ">"))

    // Output heads
    let x-gap = 3.5
    box-node((left-x - x-gap/2, y-current), x-gap, box-height,
      [$bold(h)^g_1$ #dimarr(pre-head-dim, h-dim)], fill: rgb("#ffccbc"))
    box-node((right-x - x-gap/2, y-current), x-gap, box-height,
      [$bold(z)_1^"raw"$ #dimarr(pre-head-dim, z-dim)], fill: rgb("#ffccbc"))
    ...
    box-node((x-off +box-width/4, y-current), box-width, box-height,
      [$bold(z)_t$ #dim(z-dim)], fill: rgb("#ffccbc"))
    ...
    box-node((x-center + x-off, y-current), box-width, box-height,
      align(center)[Linear + LayerNorm + GELU \ #dimarr(z-dim, 16)], fill: rgb("#fff3e0"))
    arrow-down(y-current + y-gap, y-current + box-height, x-off + box-width/2)

    y-current -= y-gap
    box-node((x-center + x-off, y-current), box-width, box-height,
      align(center)[Linear + LayerNorm + GELU \ #dimarr(16, 32)], fill: rgb("#fff3e0"))
    arrow-down(y-current + y-gap, y-current + box-height, x-off + box-width/2)

    y-current -= y-gap
    box-node((x-center + x-off, y-current), box-width, box-height,
      align(center)[Linear + LayerNorm + GELU \ #dimarr(32, 32)], fill: rgb("#fff3e0"))
    arrow-down(y-current + y-gap, y-current + box-height, x-off + box-width/2)
    ...
    y-current -= y-gap
    box-node((x-center + x-off, y-current), box-width, box-height,
      [Linear + tanh  $dot$ 5 #dimarr(32, half-dim)], fill: rgb("#b2dfdb"))
    arrow-down(y-current + y-gap, y-current + box-height, x-off + box-width/2)
    ...
    box-node((x-center + x-off + box-width/4, y-current), box-width, box-height,
      [$bold(hat(mu))_t$ #dim(half-dim)], fill: rgb("#fff3e0"))
  })
