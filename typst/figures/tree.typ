...
#let tree_figure = canvas({
  ...
  content((2*node-sep, -0.5*level-sep), text("Parenchymal Layer", style: "oblique", weight: "semibold"))
  content((3*node-sep, -1.5*level-sep), text("Immune Layer", style: "oblique", weight: "semibold"))
  // Draw nodes level by level
  // Root (level 0)
  draw-node((0, 0), "Base State", name: "p0")

  // Level 1
  draw-node((-node-sep, -level-sep), text(gray)[Pathological]+"\nOrgan Dysfunction\nFig5 B,C", name: "p1")
...
// ), name: "p1")
  ...
  // Level 2
  draw-node((0 * node-sep, -2 * level-sep), text(gray)[Vulnerable]+"\n(Suspected) Infection\nFig5 C'", name: "p3")
  draw-node((1 * node-sep, -2 * level-sep), text(gray)[Resilient]+"\n"+text(gray)[Fig5 D,E], name: "p4", full_radius: false)
  ...
  // Draw edge labels
  draw-edge-label("p0", "p1", "Desync " + $angle.l dot(phi)^1_j angle.r$)
  draw-edge-label("p0", "p2", "Sync " + $angle.l dot(phi)^1_j angle.r$, left: false)
  draw-edge-label("p2", "p5", "Sync " + $angle.l dot(phi)^2_j angle.r$, left: false)
  draw-edge-label("p2a", "p4", "Desync " + $\ angle.l dot(phi)^2_j angle.r$, left:false)
